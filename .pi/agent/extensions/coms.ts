...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerFlag("name", {
    description:
      "Override agent name (otherwise from frontmatter or auto-generated)",
    type: "string",
    default: undefined,
  });
  ...
  function renderPool(width: number, theme: Theme): string[] {
    ...
    if (safeWidth < 12) {
      ...
      const remaining =
        safeWidth - 9 /* "┏━ coms ━" */ - rightTagVisLen - 1; /* "┓" */
  ...
  pi.registerTool({
    ...
    async execute(_callId, params) {
      ...
      const promise = new Promise<{ response?: any; error?: string | null }>(
        (res, rej) => {
          resolveFn = res;
          rejectFn = rej;
        },
      ...
      if (!entry) {
        return {
          content: [
            {
              type: "text" as const,
              text: `coms_get: unknown msg_id ${params.msg_id}`,
            },
          ],
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      timeout_ms: Type.Optional(
        Type.Number({ description: "Override the default timeout (ms)." }),
      ),
    }),
    async execute(_callId, params) {
      const entry = pendingReplies.get(params.msg_id);
      if (!entry) {
        return {
          content: [
            {
              type: "text" as const,
              text: `coms_await: unknown msg_id ${params.msg_id}`,
            },
          ],
      ...
      if ((winner as any).error) {
        return {
          content: [
            {
              type: "text" as const,
              text: `coms_await: error — ${(winner as any).error}`,
            },
          ],
  ...
  pi.on("agent_end", async (_event, ctx) => {
    const inbound = [...inboundQueue.values()]
      .reverse()
      .find((i) => !i.fulfilled);
    ...
    if (
      inbound.response_schema &&
      typeof inbound.response_schema === "object"
    ) {
      try {
